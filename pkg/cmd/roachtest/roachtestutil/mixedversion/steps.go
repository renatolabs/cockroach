...
func (s startSeparateProcessVirtualClusterStep) Description() string {
	return fmt.Sprintf(
		"starting separate process virtual cluster %s with binary version %s",
		s.name, s.version,
	)
}
...
// shared-process auto-upgrading does not work reliably, and in
// separate-process deployments, where auto-upgrade is not available.
// The underlying `SET CLUSTER SETTING` command will block until the
// upgrade is finished, so this step runs in the background to allow
// user-functions to run while the migrations are in progress.
...
func (s setTenantClusterVersionStep) Description() string {
	return fmt.Sprintf(
		"run migrations on tenant %s in the background by explicitly setting cluster version to %s",
...
func (s setTenantClusterVersionStep) Run(
	...
	taskName := fmt.Sprintf("upgrade %s to '%s'", s.virtualClusterName, s.targetVersion)
	h.Background(taskName, func(ctx context.Context, l *logger.Logger) (retError error) {
