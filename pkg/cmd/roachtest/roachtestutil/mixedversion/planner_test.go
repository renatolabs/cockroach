...
func TestTestPlanner(t *testing.T) {
	...
	datadriven.Walk(t, datapathutils.TestDataPath(t, "planner"), func(t *testing.T, path string) {
		...
		defaultOpts := []CustomOption{
			EnabledDeploymentModes(NonUADeployment),
		}
...
func Test_startClusterID(t *testing.T) {
	...
	// first step of the test (ID = 1) in non-UA deployments.
	mvt := newTest(NeverUseFixtures, EnabledDeploymentModes(NonUADeployment))
	plan, err := mvt.plan()
	require.NoError(t, err)
	...
	// step of the test (ID = 2) in non-UA deployments i.e., after fixtures
	// are installed.
	mvt = newTest(EnabledDeploymentModes(NonUADeployment))
	plan, err = mvt.plan()
	require.NoError(t, err)
...
// directive.
func createDataDrivenMixedVersionTest(t *testing.T, args []datadriven.CmdArg, defaultOpts []CustomOption) *Test {
...
func Test_stepSelectorFilter(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			mvt := newBasicUpgradeTest(NumUpgrades(3), EnabledDeploymentModes(NonUADeployment))
