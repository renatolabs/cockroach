...
func Test_runSingleStep(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "panic (stacks above): runtime error: index out of range [0] with length 0")
}
...
func (tss testSingleStep) Run(_ context.Context, _ *logger.Logger, _ cluster.Cluster, _ *Helper) error {
	return tss.runFunc()
}
