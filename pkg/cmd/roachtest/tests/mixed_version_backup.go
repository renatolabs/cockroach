...
const (
	...
	// settingsTestVersion is a placeholder value for the `version` row
	// in `system.settings`. See `handleSpecialCases` for more details.
	settingsTestVersion = "<VERSION_ROW>"
)
...
func newFingerprintContents(
	ctx context.Context, l *logger.Logger, db *gosql.DB, table string,
) *fingerprintContents {
	return &fingerprintContents{ctx: ctx, logger: l, db: db, table: table}
}

// Load computes the fingerprints for the underlying table and stores
// the contents in the `fingeprrints` field.
func (fc *fingerprintContents) Load(timestamp string, _ tableContents) error {
	fc.logger.Printf("computing fingerprints for table %s", fc.table)
	query := fmt.Sprintf(
	...
	rows, err := fc.db.QueryContext(fc.ctx, query)
	if err != nil {
...
// match.
func (fc *fingerprintContents) ValidateRestore(contents tableContents) error {
...
// a RESTORE. Specifically, the `version` cluster setting in
// `system.settings` is not not expected to be the same, as it
// reflects the cluster version active in the cluster.
func (sc *systemTableContents) handleSpecialCases(
	row []interface{}, columns []string,
) ([]interface{}, error) {
	if sc.table != "system.settings" {
		return row, nil
	}

	nameIdx := sort.SearchStrings(columns, "name")
	hasName := nameIdx < len(columns) && sc.columns[nameIdx] == "name"

	if !hasName {
		return nil, fmt.Errorf("could not find column `name` on system.settings")
	}

	if str, ok := row[nameIdx].(string); !ok || str != "version" {
		return row, nil
	}

	// Replace the entire row with a sentinel value; in this test, we
...
func (sc *systemTableContents) Load(timestamp string, previous tableContents) error {
	...
	sc.logger.Printf("loading data in table %s", sc.table)
	...
	var data []byte
	if err := sc.db.QueryRowContext(sc.ctx, query).Scan(&data); err != nil {
		return fmt.Errorf("error when running query [%s]: %w", query, err)
	}
	...
	for _, r := range opaqueRows {
		...
		processedRow, err := sc.handleSpecialCases(opaqueRow, loadColumns)
		if err != nil {
	...
	for j, table := range tables {
		...
		eg.Go(func() error {
			...
			if strings.HasPrefix(table, "system.") {
				...
				contents = newFingerprintContents(ctx, l, db, table)
			}

			if err := contents.Load(timestamp, previousTableContents(j)); err != nil {
				return err
			}
			result[j] = contents
			return nil
...
func (mvb *mixedVersionBackup) runBackup(
	...
	case fullBackup:
		...
		collection = backupCollection{
			...
			nonce:   randutil.RandString(rng, nonceLen, randutil.PrintableKeyAlphabet),
		}
...
func (mvb *mixedVersionBackup) verifyBackups(
	...
	var numErrs int
	checkRestoreError := func(collection *backupCollection, err error) {
		if err == nil {
			return
		}

		numErrs++
		l.Printf("restore error %d (backup %s):\n%s", numErrs, collection.name, err)
	}
	...
	for _, collection := range mvb.collections {
		...
		checkRestoreError(collection, h.Exec(rng, restoreStmt))
		...
		for j, contents := range collection.contents {
			...
			checkRestoreError(collection, contents.ValidateRestore(restoredTableContents))
			// if err := contents.ValidateRestore(restoredTableContents); err != nil {
	...
	if numErrs > 0 {
		return fmt.Errorf("%d errors during restore (see logs)", numErrs)
	}
	return nil
}
