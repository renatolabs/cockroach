...
func TestFingerprintValidator(t *testing.T) {
	...
	t.Run(`empty`, func(t *testing.T) {
		...
		v, err := NewFingerprintValidator(withSQLDB, `foo`, `empty`, []string{`p`}, testColumns, nil, nil)
		require.NoError(t, err)
		noteResolved(t, v, `p`, ts[0])
		assertValidatorFailures(t, v)
	})
	t.Run(`wrong data`, func(t *testing.T) {
		...
		v, err := NewFingerprintValidator(withSQLDB, `foo`, `wrong_data`, []string{`p`}, testColumns, nil, nil)
		require.NoError(t, err)
	...
	t.Run(`all resolved`, func(t *testing.T) {
		...
		v, err := NewFingerprintValidator(withSQLDB, `foo`, `all_resolved`, []string{`p`}, testColumns, nil, nil)
		require.NoError(t, err)
	...
	t.Run(`rows unsorted`, func(t *testing.T) {
		...
		v, err := NewFingerprintValidator(withSQLDB, `foo`, `rows_unsorted`, []string{`p`}, testColumns, nil, nil)
		require.NoError(t, err)
		noteRow(t, v, ignored, `[1]`, `{"after": {"k":1,"v":3}}`, ts[3])
		noteRow(t, v, ignored, `[1]`, `{"after": {"k":1,"v":2}}`, ts[2])
		noteRow(t, v, ignored, `[1]`, `{"after": {"k":1,"v":1}}`, ts[1])
	...
	t.Run(`missed initial`, func(t *testing.T) {
		...
		v, err := NewFingerprintValidator(withSQLDB, `foo`, `missed_initial`, []string{`p`}, testColumns, nil, nil)
		require.NoError(t, err)
		noteResolved(t, v, `p`, ts[0])
	...
	t.Run(`missed middle`, func(t *testing.T) {
		...
		v, err := NewFingerprintValidator(withSQLDB, `foo`, `missed_middle`, []string{`p`}, testColumns, nil, nil)
		require.NoError(t, err)
		noteResolved(t, v, `p`, ts[0])
		noteRow(t, v, ignored, `[1]`, `{"after": {"k":1,"v":1}}`, ts[1])
	...
	t.Run(`missed end`, func(t *testing.T) {
		...
		v, err := NewFingerprintValidator(withSQLDB, `foo`, `missed_end`, []string{`p`}, testColumns, nil, nil)
		require.NoError(t, err)
		noteResolved(t, v, `p`, ts[0])
		noteRow(t, v, ignored, `[1]`, `{"after": {"k":1,"v":1}}`, ts[1])
		noteRow(t, v, ignored, `[1]`, `{"after": {"k":1,"v":2}}`, ts[2])
	...
	t.Run(`initial scan`, func(t *testing.T) {
		...
		v, err := NewFingerprintValidator(withSQLDB, `foo`, `initial_scan`, []string{`p`}, testColumns, nil, nil)
		require.NoError(t, err)
		noteRow(t, v, ignored, `[1]`, `{"after": {"k":1,"v":3}}`, ts[3])
	...
	t.Run(`unknown partition`, func(t *testing.T) {
		...
		v, err := NewFingerprintValidator(withSQLDB, `foo`, `unknown_partition`, []string{`p`}, testColumns, nil, nil)
		require.NoError(t, err)
	...
	t.Run(`resolved unsorted`, func(t *testing.T) {
		...
		v, err := NewFingerprintValidator(withSQLDB, `foo`, `resolved_unsorted`, []string{`p`}, testColumns, nil, nil)
		require.NoError(t, err)
		noteRow(t, v, ignored, `[1]`, `{"after": {"k":1,"v":1}}`, ts[1])
		noteResolved(t, v, `p`, ts[1])
		noteResolved(t, v, `p`, ts[1])
	...
	t.Run(`two partitions`, func(t *testing.T) {
		...
		v, err := NewFingerprintValidator(withSQLDB, `foo`, `two_partitions`, []string{`p0`, `p1`}, testColumns, nil, nil)
		require.NoError(t, err)
		noteRow(t, v, ignored, `[1]`, `{"after": {"k":1,"v":1}}`, ts[1])
		noteRow(t, v, ignored, `[1]`, `{"after": {"k":1,"v":2}}`, ts[2])
