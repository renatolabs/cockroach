...
if systemctl is-active -q "${TENANT_LABEL}"; then
  echo "${TENANT_LABEL} service already active"
  echo "To get more information: systemctl status ${TENANT_LABEL}"
  exit 1
fi

# If cockroach failed, the service still exists; we need to clean it up before
# we can start it again.
sudo systemctl reset-failed "${TENANT_LABEL}" 2>/dev/null || true
